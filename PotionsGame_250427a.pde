...
Animation stirring, playerSprite, transformation;
color redCol = #FF1212, blueCol = #3412FF, lightPurp = #D17FD6, gold = #C6BA4C;
UI ui;
Player player;
boolean isHovering, pickPotion, canDrop, grabbing, transforming=false;
Objects[] objects;
PImage[] potion;
PVector imageSize, lastPOS;
;
float size=100;
...
void setup() {
  ...
  playerSprite= new Animation ("player", 10);
  transformation = new Animation ("Transformation0", 10);
...
void draw() {
  ...
  player.run();
      loadImages();
  if (!transforming) {
    playerSprite.spriteDisplay();
  }if (transforming){
    transformation.transformDisplay();
    transforming= false;
    resetObjects();
  }
  ui.run();
